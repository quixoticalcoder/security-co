...
export async function generateRunReportPdf(detail: RunDetail): Promise<void> {
  ...
  doc.text('Security Co', MARGIN, 14)
  ...
  for (let p = 1; p <= pageCount; p++) {
    ...
    doc.setFontSize(7.5)
    doc.setTextColor(...COLOR.muted)
    doc.text('Generated by Security Co — automated analysis, not a substitute for human judgment.', MARGIN, FOOTER_Y)
  ...
  doc.save(`security-co-${filenameBase || detail.id}.pdf`)
}
