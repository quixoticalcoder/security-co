...
class SecurityCoError(Exception):
    """Base class for all domain-level errors raised by Security Co services."""
...
class NotFoundError(SecurityCoError):
    status_code = 404
    error_code = "not_found"


class AlreadyExistsError(SecurityCoError):
    status_code = 409
    error_code = "already_exists"


class InvalidCredentialsError(SecurityCoError):
    status_code = 401
    error_code = "invalid_credentials"


class UnauthorizedError(SecurityCoError):
    status_code = 401
    error_code = "unauthorized"


class ForbiddenError(SecurityCoError):
    status_code = 403
    error_code = "forbidden"


class ValidationError(SecurityCoError):
    status_code = 422
    error_code = "validation_error"


class ExternalServiceError(SecurityCoError):
...
class RateLimitedError(SecurityCoError):
    status_code = 429
