...
The Security Co backend. Flat, one-concern-per-file layout — no
`app/` wrapper, no ORM, no auth. See project memory
`security-co-spec` for the full build spec and
`security-co-poc-scope` for what's deliberately cut for this POC
