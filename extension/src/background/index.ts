/**
 * Security Co background service worker.
 *
...
async function syncBlocklist(): Promise<void> {
  try {
    ...
    // probably accurate.
    console.warn("Security Co: blocklist sync failed", error);
  }
}
...
async function sendToTab(tabId: number, message: BackgroundToContentMessage): Promise<void> {
  for (let attempt = 0; ; attempt++) {
    ...
    console.warn("Security Co: sendToTab gave up after retries", tabId, message.type, error);
        return;
      }
...
function reasonFor(result: QuickCheckResponse): string {
  switch (result.source) {
    case "blocklist":
      return "This URL matches Security Co's static blocklist of known-bad domains.";
    case "virustotal":
    ...
    default:
      return "Security Co's local phishing-URL model flagged this address. Click \"Full report\" for a complete investigation.";
  }
}
...
async function applyQuickCheckResult(tabId: number, url: string, result: QuickCheckResponse): Promise<void> {
  ...
  if (result.label === "dangerous" || result.label === "suspicious") {
    ...
    await setTabVerdict(tabId, {
      ...
      label: "safe",
      confidence: result.confidence,
      reason: "Security Co's automatic quick scan found nothing suspicious about this page.",
      mitigation: null,
      legitimateAlternatives: [],
      runId: null,
      checkedAt: Date.now(),
...
async function scanNavigation(tabId: number, url: string): Promise<void> {
  ...
  try {
    ...
    console.warn("Security Co: quick-check failed", error);
  }
}
...
async function handlePageSignals(tabId: number, url: string, actionDomain: string): Promise<void> {
  ...
  try {
    ...
    console.warn("Security Co: page-signal check failed", error);
  }
}
