/**
 * Security Co content script — the in-page banner.
 *
 ...
 */
(function securityCoContentScript() {
  type Label = "dangerous" | "suspicious" | "safe";

  const HOST_ID = "security-co-banner-host";
  ...
  function showBanner(url: string, label: Label, confidence: number, source: string): void {
      ...
      card.innerHTML = `
        <div class="row">
          ...
          <div>
            ...
            <div class="sub">Security Co &middot; quick scan</div>
          </div>
        </div>
      `;
    ...
    card.innerHTML = `
      ...
      <div class="row">
        ...
        <div>
          <div class="title ${label}">${labelTitle(label)}</div>
          <div class="sub">Security Co &middot; ${Math.round(confidence * 100)}% confidence &middot; quick scan</div>
        </div>
      </div>
